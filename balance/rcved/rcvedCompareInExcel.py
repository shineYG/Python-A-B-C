...
try:
    ...
    wb = openpyxl.load_workbook('rcved.xlsx')
    ws = wb.active
    #按年、月汇总水费
    ...
    # 加载新系统应收sql语句
    sql = ""
    with open("sql-rcved.txt", "r", encoding="utf-8") as f:  
        sql = f.read()
    ...
    for y in yearSumInDB:
        if (yearSumInExcel[y] - yearSumInDB[y]) != 0:
            ...
            diffFee.append((y, str(yearSumInExcel[y]), str(yearSumInDB[y]), str(round(yearSumInExcel[y] - yearSumInDB[y], 2))))   
        else:
